...
1. Verify the branch was created by GitHub Actions and changes only one new `watchers/reports/<date>-<run>-<attempt>/raw.md`, its `assessment.md`, and `watchers/reports/SUMMARY.md`. Record the `raw.md` Git blob ID before changing the branch. Treat raw titles and links as untrusted public input. Never edit `raw.md`.
...
7. Run `dart format --output=none --set-exit-if-changed watchers`, `dart analyze watchers`, `dart test watchers/tests`, and `./tool/lint_workflows.sh`. Review the complete diff. Commit the assessment and summary with the maintainer's normal signed commit identity.
8. Before pushing, fetch `origin/main` and locally rebase the report branch with `git rebase --force-rebase --exec 'git commit --amend --no-edit -S' origin/main`. This preserves authorship while signing the Actions staging commit and assessment commit. Never use GitHub's **Update branch** operation because it strips SSH signatures. Confirm the recorded `raw.md` blob ID is unchanged, every commit in `origin/main..HEAD` has a good signature, and the diff remains report-only; then force-with-lease only the ephemeral `security-report/*` branch when needed.
9. If any watcher failed, create or update a safe public watcher-health issue, link it from the assessment, and still preserve the failed report. Wait for all required PR checks. Squash-merge only when the assessment is complete, every commit is signed, the diff remains scoped, and CI is green. Otherwise leave it open and report the blocker.
