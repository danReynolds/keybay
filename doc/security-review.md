...
| KB-SA-04 | Low | In JIT development builds, SIGQUIT can start the Dart VM service mid-session, and the Keychain item trusts the Dart toolchain's signature. | Accepted: release AOT builds have neither. On 2026-09-24 the CLI's pub.dev channel was dropped, so it ships only as release builds through Homebrew and GitHub releases. |
