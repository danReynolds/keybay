...
| KB-SA-02 | Medium | A crash could write an open session's store key and a revealed value into a core file. | Fixed: every command disables core files, and on Linux the process is non-dumpable. |
| KB-SA-03 | Medium | macOS Copy was eligible for Universal Clipboard and clipboard-history recording. | Fixed on macOS: the copy is current-host-only and carries the concealed and transient markers. The Linux helpers cannot carry a sensitivity hint; documented. |
...
| KB-SA-07 | Low | Some Unicode format characters were not escaped, and a prepend cluster could vanish from the display. | Fixed in Keybay: format and default-ignorable characters and zero-width clusters are escaped, and `get` refuses the same set. Fleury width fix: [danReynolds/fleury#269](https://github.com/danReynolds/fleury/pull/269). |
