...
| Windows | Rides the core `WinCredApi` backend ([implementation-plan.md](implementation-plan.md) Phase 5). No interim plaintext-key-on-disk scheme — that would be S4 storage behind a product that promises S3+ (design.md §9), violating fail-closed. |
| Headless / CI operation | There is no dedicated headless backend or reliable headless detector. The CLI is a dev-machine tool; if its required OS credential service is unavailable it returns typed guidance, and CI should use the CI platform's secret store. |
...
| dotenvx | ciphertext in repo; keys movable to OS keychain | ✅ (encrypted values) | ✅ | ✅ | Different model: ciphertext is designed to live in the repository. |
