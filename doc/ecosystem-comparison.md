...
|---|---|---|---|
| **Keybay** | Per-developer local values; Dart/Flutter access; austere macOS/Linux launch injection | Per-app SDK stores; one CLI application store with simple or namespaced keys; fixed OS-backed runtime policy; no sync | A session-based SDK, or six CLI commands including the interactive vault and one small manifest |
