...
void main() {
  ...
  test('records one clean source-scoped physical configuration', () async {
    ...
    final report =
        jsonDecode(File('${temp.path}/report.json').readAsStringSync())
            as Map<String, dynamic>;
    expect(report['schema'], 'keybay.device-security-report');
    expect(report['schema_version'], 1);
    expect(report['status'], 'pass');
    expect(report['source'], {'commit': source.split(':').last, 'clean': true});
    ...
    expect(
      (report['scenarios'] as List<dynamic>).cast<Map<String, dynamic>>().every(
  ...
  test(
    'parser prevents an aggregate Flutter failure from becoming a pass',
    () async {
          ...
          for (var index = 0; index < 3; index++) ...[
            _event('testStart', {
              'test': {
                'id': index + 2,
                'name':
                    'group ${['KB-AND-010', 'KB-AND-011', 'KB-AND-020'][index]} test',
              },
            }),
            _event('testDone', {
              'testID': index + 2,
  ...
  test('failed command or cleanup cannot become a pass report', () async {
    ...
    final scenario = await runReport([], output: '${temp.path}/scenario.json');
  ...
  test(
    'rejects substituted source, dirty source, identifiers, and overwrite',
    () async {
      _writeResults(
        results,
      ...
      expect(
        (await runReport([
          '--field',
          'deviceId=private',
        ], output: '${temp.path}/identifier.json')).exitCode,
        64,
      );
      ...
      File(
        '${cleanToolRepo.path}/tool/device_security/catalog.dart',
  ...
  test('catalog inventory, procedures, and normative guarantees agree', () async {
    final result = await Process.run(Platform.resolvedExecutable, [
      '$repo/tool/device_security/catalog.dart',
      'markdown',
    ], workingDirectory: repo);
    expect(result.exitCode, 0, reason: '${result.stderr}');
  ...
  test('iOS selector records model and OS without the raw device ID', () async {
    final process = await Process.start(Platform.resolvedExecutable, [
      '$repo/tool/device_security/flutter_device.dart',
      'private-device-id',
    ], workingDirectory: repo);
    process.stdin.write(
      jsonEncode({
        'flutter': [
          {
            ...
            'name': 'private-personal-device-name',
            'targetPlatform': 'ios',
            'emulator': false,
            'isSupported': true,
            'sdk': 'iOS 26.1',
    ...
    expect(stdoutText, isNot(contains('private-personal-device-name')));
  });

  test(
    'iOS selector rejects absent Xcode identity without reflecting input',
    () async {
      final process = await Process.start(Platform.resolvedExecutable, [
        '$repo/tool/device_security/flutter_device.dart',
        'private-device-id',
  ...
  test(
    'entrypoint and platform adapters retain destructive safety guards',
    () async {
      final traversal = await Process.run('bash', [
        '$repo/tool/device_security.sh',
        'doctor',
        '../device_security/linux',
      ], workingDirectory: repo);
      expect(traversal.exitCode, 64);
      final unportedMac = await Process.run('bash', [
        '$repo/tool/device_security.sh',
        'run',
        'macos',
      ], workingDirectory: repo);
      expect(unportedMac.exitCode, 64);
      expect(unportedMac.stderr, contains('has not been ported'));
      expect(
        File('$repo/tool/device_security/macos.sh').readAsStringSync(),
        isNot(contains('delete-generic-password')),
      );

      final android = File(
        '$repo/tool/device_security/android.sh',
      ).readAsStringSync();
  ...
  test(
    'Android backup safety is intrinsic and macOS identity stays scoped',
    () {
      final manifest = File(
