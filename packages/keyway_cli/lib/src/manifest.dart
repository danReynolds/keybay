...
/// Parses arbitrary bytes as a Keyway manifest or throws a typed, non-echoing
...
Manifest parseManifestBytes(List<int> source) {
  ...
  for (var index = 0; index < lines.length; index++) {
    ...
    if (value.startsWith('kw://')) {
      final key = value.substring('kw://'.length);
      if (!isValidCliKey(key)) {
        throw ManifestParseException(
          'reference must look like '
          'kw://acme-payments/openai-api-key and be at most '
