...
import 'package:keyway_cli/src/manifest.dart';
  ...
  group('manifest parser', () {
    test('parses literals, references, comments, and empty values', () {
      final manifest = _parse(
        ...
        'SHARED=kw://acme-shared/service/key\n',
      );
...
''');
    ...
    test(
      'rejects every malformed kw reference instead of making it literal',
      () {
  ...
  test('readManifest bounds the file read', () async {
    final directory = await Directory.systemTemp.createTemp('keyway-manifest-');
