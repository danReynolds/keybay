...
import 'package:keyway_cli/src/failure.dart';
...
void main() {
  ...
  test('unsafe-permission remediation shell-quotes the path', () {
    final failure = failureForSecretStore(
      SecureFileError('read(insecure-mode:644)', "/tmp/keyway user's/store", 0),
    );
    expect(
      failure.lines.last,
      r"Restrict it and retry: chmod 600 -- '/tmp/keyway user'\''s/store'",
    );

    final directoryFailure = failureForSecretStore(
      SecureFileError('insecure-dir-mode(755)', "/tmp/keyway user's", 0),
    );
    expect(
      directoryFailure.lines.last,
      r"Restrict it and retry: chmod 700 -- '/tmp/keyway user'\''s'",
    );
