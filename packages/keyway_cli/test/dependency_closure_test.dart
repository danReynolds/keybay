...
void main() {
  ...
  test('runtime dependency closure is the vetted set', () {
    ...
    final root = packages.firstWhere(
      (package) => package['name'] == 'keyway_cli',
    );
    final seeds = (root['directDependencies'] as List).cast<String>();
    expect(seeds, unorderedEquals(<String>['ffi', 'keyway']));
    ...
    for (final name in closure) {
      ...
      if (name == 'keyway') {
        expect(
          source,
          'root',
          reason: 'keyway must resolve from the workspace',
        );
      } else {
        expect(
          source,
  ...
  test('runtime dependencies are exact-pinned without overrides', () {
    ...
    expect(
      pubspec,
      contains(RegExp(r'^\s*keyway:\s*0\.1\.0\s*$', multiLine: true)),
    );
    expect(
      pubspec,
...
Directory _packageDirectory() {
  final nested = Directory('${Directory.current.path}/packages/keyway_cli');
