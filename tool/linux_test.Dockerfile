...
RUN apt-get update -qq && apt-get install -y --no-install-recommends ca-certificates flatpak xdg-desktop-portal gnome-keyring dbus libglib2.0-bin python3 ruby sudo procps bubblewrap gcr xvfb xdotool xclip xauth git && rm -rf /var/lib/apt/lists/*
